...
//     alert(`تم حفظ الوظيفة: ${jobtitle}`); // Corrected to use template literals; // Corrected to use template literals
//   } else {
//     alert("this jop has been saved!"); // Changed to Arabic for consistency
//   }
// };
const saveJob = (jobTitle) => {
  let savedJobs = JSON.parse(localStorage.getItem("savedJobs")) || [];
  if (!savedJobs.includes(jobTitle)) {
    savedJobs.push(jobTitle);
    localStorage.setItem("savedJobs", JSON.stringify(savedJobs));
    alert(`تم حفظ الوظيفة: ${jobTitle}`); // Use the parameter directly
  } else {
    alert("هذه الوظيفة محفوظة مسبقاً!"); // Arabic message
  }
};
